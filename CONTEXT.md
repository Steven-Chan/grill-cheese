...
- **Card swap** — animated transition between BigCard surfaces. Layered-stack metaphor with directional grammar: **forward** (commit → next, summary verdict, continue_grill resume) = old card slides OUT to the LEFT, new card slides IN from the RIGHT; **backward** (sidebar history pin, `← back to current question`) mirrors — old slides RIGHT, new slides IN from the LEFT. Sequential beat: exit → brief pause → enter card-slide, then inner stagger fades head → branches (one-by-one) → composer + actions. `prefers-reduced-motion` collapses to instant cut. First BigCard mount per session (page reload, cmd+P swap) renders the active node in final state without animation; only subsequent swaps animate. Distinct from the **Lock transition** (commit-moment phase 1+2), which runs on the old card BEFORE the swap begins and is preserved through the slide. Concrete durations / easings / delays live in `gui/src/styles.css` (`gc-card-exit`, `gc-card-enter`, `gc-card-inner` keyframes); the doc deliberately omits numbers — they are tuning knobs, not contract. See ADR-0008.
...
- **Parked slot** — server-side payload (`{slot_id, question, reasoning, branches[full with rationales+is_recommended], multi_select}`) generated by the speculator teammate before its consume moment. Lives on `Session.parked_queue`. Cleared on session end. NO `progress` field — main supplies it at `present_parked` time.
- **Speculation queue** — `Session.parked_queue: list[ParkedSlot]`. Dynamic depth K∈[1,5], teammate-sized per refresh. Cleared on session end. Server is dumb storage; no routing intelligence.
- **Sideways routing** — Claude's choice on commit-wake: dequeue a parked slot (sideways move from brief) when current answer does not dictate a drill-down.
- **Live-eval routing** — main Claude's per-wake judgment: drill-down → live-generate via `present_branches`; else → `present_parked(slot_id)`. Queue is fallback, never default; preserves grill depth.
- **Speculator teammate** — long-lived subagent (Agent tool, name=`speculator-<session_id>`, spawned at session start, own context window). Receives `SendMessage` triggers from main each push; calls `enqueue_speculation` directly. Main never holds parked payloads in its own context.
- **`enqueue_speculation`** — MCP tool, teammate-only. Replaces the current queue contents for a session.
- **`present_parked`** — MCP tool, main-only. Promotes a `slot_id` from the queue to the active decision node. Server broadcasts `node_committed`. Skips main's generation. Signature: `present_parked(session_id, slot_id, progress: float)`. Naming parallels `present_branches` / `present_summary`; `commit_parked` was rejected (overloads user-action "commit").
- **Channel parked_slots hint** — each `node_committed` channel payload gains `parked_slots: [{slot_id, question_oneline}]`. Lightweight; main reads to know what's parked without surviving compaction.
...
- "Progress" on parked content: not part of the parked payload; main re-estimates honestly at `present_parked` time. Honest-shrink semantics from ADR-0007 preserved on sideways consume.
